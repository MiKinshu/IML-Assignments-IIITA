...
def getscaleddata():
	...
	# Converting the data to 0 - 1 range.
	minmarks1 = np.min(marks1)
	maxmarks1 = np.max(marks1)
	marks1 = (marks1 - minmarks1) / (maxmarks1 - minmarks1)
	maxmarks2 = np.max(marks2)
	minmarks2 = np.min(marks2)
	marks2 = (marks2 - minmarks2) / (maxmarks2 - minmarks2)

	#applying box-muller transformation
	marks1temp = copy.deepcopy(marks1)
	marks2temp = copy.deepcopy(marks2)
	for i in range(len(marks1)):
		if marks1[i] == 0:
			continue
		marks1temp[i] = (math.sqrt(-2 * math.log(marks1[i])) * math.cos((2 * 3.14) * marks2[i]))

	for i in range(len(marks1)):
		if marks1[i] == 0:
			continue
		marks2temp[i] = (math.sqrt(-2 * math.log(marks1[i])) * math.sin((2 * 3.14) * marks2[i]))
